...
class SuggestionStripView(context: Context, attrs: AttributeSet?, defStyle: Int) :
    ...
    override fun onDetachedFromWindow() {
        aiVoiceToolbarCollection?.cancel()
        aiVoiceToolbarCollection = null
    ...
    override fun onAttachedToWindow() {
        ...
        aiVoiceToolbarCollection?.cancel()
        runCatching { AiVoiceDependencies.get() }.getOrNull()?.let { dependencies ->
            aiVoiceToolbarCollection = aiVoiceToolbarScope.launch {
                dependencies.runtimeState.state.collectLatest { state ->
    ...
    private fun addKeyToPinnedKeys(pinnedKey: ToolbarKey) {
        ...
        val copy = createToolbarKey(context, pinnedKey)
        ...
        updateAiVoiceToolbarVisuals(copy)
    ...
    private fun setupKey(view: ImageButton, colors: Colors) {
        ...
        if (view is AiVoiceToolbarButton) view.setLabelColor(colors.get(ColorType.TOOL_BAR_KEY))
