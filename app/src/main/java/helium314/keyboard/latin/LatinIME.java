...
import helium314.keyboard.latin.aivoice.runtime.AiVoiceImeBridge;
import helium314.keyboard.latin.aivoice.runtime.AiVoiceSessionController;
...
public class LatinIME extends InputMethodService implements
    ...
    public void onCreate() {
        ...
        try {
            mAiVoiceSessionController = AiVoiceImeBridge.create(this);
        } catch (Exception e) {
    ...
    public void onEvent(@NonNull final Event event) {
        if (KeyCode.DESH_NO_INPUT_VOWEL == event.getKeyCode()) {
            ...
            resetDeshHindiVowelDiacriticMode();
            return;
        }
        if (KeyCode.DESH_TRANSLATE == event.getKeyCode()) {
            toggleDeshTranslation();
            return;
        }
        if (mDeshTranslationView != null && mDeshTranslationView.isOpen()
                && mDeshTranslationView.handleKeyCode(event.getKeyCode())) {
            return;
        }
        ...
        if (KeyCode.AI_VOICE_PREVIOUS_PROFILE == event.getKeyCode()
                || KeyCode.AI_VOICE_NEXT_PROFILE == event.getKeyCode()) {
            if (mAiVoiceSessionController != null)
                mAiVoiceSessionController.onManualProfileStepRequested(
                        KeyCode.AI_VOICE_NEXT_PROFILE == event.getKeyCode());
            return;
        }
        if (mAiVoiceSessionController != null)
    ...
    public void onAiVoiceInputInteraction() {
        if (mAiVoiceSessionController != null)
            mAiVoiceSessionController.onInputInteraction();
    }
